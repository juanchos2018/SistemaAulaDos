        ...
        <template #cell(actions)="data">
          <div class="text-nowrap">
            <feather-icon
              ...
              size="16"
              @click="modalNotificacion(data.item.SEC_ID)"
            />
            <b-tooltip
...
export default {
 components: {
  ...
  methods: {
    ...
    vsnotifys(){
      let me = this;     
      let url = "/vsnotify/records/?"+me.getQueryParameters();
      axios({
        method: "GET",
        url: url,
      }).then((response) => {    
    ...
    exportExcel(){
       let url = "/vsnotify/excel/";
       window.open(url, "_blank");
    },
    exportPdf  ()  {
      let url = "/vsnotify/pdf/";
      window.open(url, "_blank");
    }
  },
};
